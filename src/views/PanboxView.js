...
export default class PanboxView extends HTMLElement {
	...
	set scrollTop(value) {
		return super.scrollTop = this._scrollTop = value;
	}
	...
	set scrollLeft(value) {
		return super.scrollLeft = this._scrollLeft = value;
	}
	...
	setViewbox(x, y, w, h) {
		...
		this.scrollLeft = Math.max(0, Math.min(x * this.scale, this.scrollWidth));
		this.scrollTop = Math.max(0, Math.min(y * this.scale, this.scrollHeight));
		
		clearTimeout(this.setViewbox._tmrDisableIgnoreScroll);
		this.setViewbox._tmrDisableIgnoreScroll = setTimeout(() => {
	...
	$$onScroll(event) {
		...
		this._scrollLeft = super.scrollLeft;
		this._scrollTop = super.scrollTop;
